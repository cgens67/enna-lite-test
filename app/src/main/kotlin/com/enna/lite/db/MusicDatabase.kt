...
import com.enna.lite.db.entities.PlaylistSongMapPreview
...
import com.enna.lite.db.entities.SortedSongAlbumMap
import com.enna.lite.db.entities.SortedSongArtistMap
...
private object SchemaTools {
    ...
    private fun ensureTableSchema(
        ...
        if (!schemaMismatch(expectedColumns, actualColumns)) {
            expectedIndices.filter { it.tblName == table.name }.forEach { it.sql?.let { sql -> db.execSQL(sql) } }
            return
        }
